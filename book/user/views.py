        ...
        if form.is_valid():
            ...
            if result:
                ...
                if user.password == password:
                    ...
                    return render(
                        request, "user/login.html", {"form": form, "error": "Incorrect account or password!"}
                    )
            else:
                return render(
                    request, "user/login.html", {"form": form, "error": "Account does not exist!"}
                )
    else:
        form = Login()
...
def all_book(request):
    ...
    return render(request, "user/item.html", {"books": books, "title": "All Courses"})
...
def personal(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request, "user/personal.html", {"message": "Failed try", "form": form}
            )
...
def hot_book(request):
    ...
    return render(request, "user/item.html", {"books": books, "title": "Hottest Courses"})


def latest_book(request):
    page_number = request.GET.get("page", 1)
    books = books_paginator(Book.objects.order_by("-id")[:10], page_number)
    return render(request, "user/item.html", {"books": books, "title": "Newest Courses"})
...
def create_book(request, ):
    ...
    with open('book_2.sql', 'r', encoding='utf-8') as f:
